"""Aggregates West Virginia-located listings from every taildraggers.com
...
ones whose location ends in ", WV".
...
Vermont/Virginia/Washington repos' approach, with the state filter swapped
to "WV".
"""
...
PAGE_TITLE = "Taildragger Ads in West Virginia"
STATE_ABBR = "WV"
...
_WEST_VIRGINIA_LOCATION_RE = re.compile(r",\s*WV\s*$", re.IGNORECASE)


def _is_west_virginia(location: str) -> bool:
    return bool(_WEST_VIRGINIA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        west_virginia_rows = [row for row in rows if _is_west_virginia(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(west_virginia_rows)} in West Virginia")
        all_listings.extend(west_virginia_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No West Virginia listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique West Virginia listings: {len(listings)}")
