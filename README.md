...
Daily aggregator of West Virginia-located taildragger classified listings,
...
[Washington](https://github.com/taildraggers/washington) repos' approach,
with the state filter swapped to "WV".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, WV` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in West Virginia"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/west-virginia/`
...
```html
<iframe
  src="https://taildraggers.github.io/west-virginia/"
  title="Taildragger Ads in West Virginia"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_WEST_VIRGINIA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
